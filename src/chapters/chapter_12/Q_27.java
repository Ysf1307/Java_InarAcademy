...
public class Q_27 {
    ...
    public static void replaceWords(ArrayList<File> file) throws FileNotFoundException {
        for (int i = 0; i < file.size(); i++) {
            ...
            try (Scanner input = new Scanner(file.get(i))){
                while (input.hasNext()){
            ...
            try (PrintWriter output = new PrintWriter(file.get(i))){
    ...
    public static void addFiles(ArrayList<File> list, String[] args){
        for (int i = 0; i < args.length; i++) {
            if (!args[i].equals("Exercise_12_27.java") && args[i].equals("Exercise_12_27.class")){
